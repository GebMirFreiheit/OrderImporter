...
def get_orders():
    ...
    resp = service.spreadsheets().values().get(
        spreadsheetId='1Qkuo4NopE1XbEO0TbeAUpI-QVXcxDKlJpnOEExFghgs', range="Лист1!B2:Z1000").execute()

    rows = resp['values']
    # список номеров актуальных заказов (т.е. тех, которые на момент запроса есть в таблице)
    actual_data = []
    ...
    data = [dict(zip(['order_number', 'dollar_price', 'delivery_time'], row))
            for row in rows]
